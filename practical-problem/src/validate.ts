...
export class FormFieldState {
  constructor(public valid: ValidStatus, public changed: ChangedStatus) {}
}
...
export class Validators {
  ...
  static pattern: ValidatorFactory =
    (validationPattern: RegExp) => (value: string, field: string) => {
      if (!validationPattern.test(value)) {
        throw `The field '${field}' does not match pattern '${validationPattern}'`;
      }
    };
  static len: ValidatorFactory =
    (min: number, max: number) => (value: string, field: string) => {
      if (value.length < min) {
